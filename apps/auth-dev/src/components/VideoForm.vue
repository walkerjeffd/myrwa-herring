        ...
        <div class="field-list clear">
          <div id="" class="form-item field number">
            ...
            <!-- <input
              ...
              v-model="form.count"
              @keydown.enter="submit">
            <span v-show="errors.has('count')" class="help is-danger">
              {{ errors.first('count') }}
            </span> -->
          </div>
...
export default {
  ...
  methods: {
    submit() {
      ...
      axios.post('/count/', payload)
        .then(() => {
          this.$store.dispatch('incrementSessionCounts');
          this.$store.dispatch('incrementSessionTotal', payload.count);
