...
grid.arrange(p1, p2, p3, p4, ncol = 2, top = "Mystic River Herring | Hourly Video Summary", bottom = paste0("Updated: ", now()))
...
p <- daily %>%
  ...
  labs(
    x = "Date",
    y = "# Videos",
    title = "Number of Videos per Video Day"
  )
...
p1 <- daily %>%
  ...
  labs(
    x = "Date",
    y = "# Counts",
    title = "Number of Counts per Video Day"
  )
...
p2 <- daily %>%
  ...
  labs(
    x = "Date",
    y = "# Fish",
    title = "Number of Fish Counted per Video Day"
  )
